...
int showNonMatches(cv::Mat, cv::Mat, vector<Point2f>, float,string,ofstream&,std::vector<Rect>&);
float ransacCompare(cv:: Mat, cv:: Mat, string, int,ofstream&,int,int,bool,Rect,Rect);
...
//PARAMETER CHANGE
    ...
    while(1){
	    ...
	    string instanceName = "instance_" + tStampIns;
        ...
        imwrite(planoInstance,planogramCol);
        ...
        string tStampQuery = getDateTimeStamp();
        querySrc = frame.clone();
        Mat queryCol = querySrc.clone();
        string queryInstance = destSubDirPath + "query_" + tStampQuery + ".jpg";
        ...
        int integrity = showNonMatches(planogramSrc,querySrc,nonMatchVector,ImgResize,instanceName,myfile,boundBox); 
...
int showNonMatches(cv::Mat imgMatRaw, cv::Mat imgMatRaw_2, vector<Point2f> nonMatchVector, 
    float ImgResize, string instanceName,ofstream& myfile,std::vector<Rect>& boundBox){
    ...
    for(int i = 1; i < stats.rows; i++){
        ...
        if(hogArea > 1200 && hogArea <= 42000 && hogAreaRatio > 0.20){
            ...
            float ratio_plano_to_query = ransacCompare(ransacPlano,ransacQuery,instanceName,i,myfile,Area,hogArea,plano2query,plan_rect_bound,query_rect_bound);
            ...
            float ratio_query_to_plano = ransacCompare(ransacPlano,ransacQuery,instanceName,i,myfile,Area,hogArea,plano2query,plan_rect_bound,query_rect_bound);
...
float ransacCompare(cv:: Mat img_1, cv:: Mat img_2, string instanceName, int imgCount,ofstream& myfile,int Area,int hogArea,bool plano2query, Rect plan_rect_bound, Rect query_rect_bound){
    ...
    myfile << instanceName << ','  << imgCount << ',' << hogArea << ',' << Area << ',';
