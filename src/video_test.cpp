...
float ransacCompare(cv:: Mat, cv:: Mat, string, int,ofstream&,int,int,int,string,bool);
...
//PARAMETER CHANGE
    ...
    for (unsigned int i = 0;i < 1;i++){
        ...
        imwrite("../plano.jpg",planogramCol);
...
int showNonMatches(cv::Mat imgMatRaw, cv::Mat imgMatRaw_2, vector<Point2f> nonMatchVector, 
    ...
    for(int i = 1; i < stats.rows; i++){
        ...
        if(hogArea > 700 && hogArea < 26000 && hogAreaRatio > 0.20){
            ...
            float ratio_plano_to_query = ransacCompare(ransacPlano,ransacQuery,videoFolder1,i,myfile,Area,hogArea,frame_no,subDir,plano2query);
            plano2query = false;
            string videoFolder2 = videoFolder + '/' + "query2plano/";
            makeAfolder(videoFolder2);
            float ratio_query_to_plano = ransacCompare(ransacQuery,ransacPlano,videoFolder2,i,myfile,Area,hogArea,frame_no,subDir,plano2query);

            bool noChange = (((ratio_plano_to_query == -1) || (ratio_plano_to_query > 0.7)) && ((ratio_query_to_plano == -1) || (ratio_query_to_plano > 0.7))); 
...
float ransacCompare(cv:: Mat img_1, cv:: Mat img_2, string videoFold, int imgCount,ofstream& myfile,int Area,int hogArea,int frame_no,string subDir, bool plano2query){
