...
int main(int argc, char** argv){
	...
	Mat ransacPlano = imread(filename1,IMREAD_GRAYSCALE);

    copyMakeBorder(ransacPlano,ransacPlano,50,50,50,50,BORDER_CONSTANT,255);
    //resize(ransacPlano,ransacPlano,Size(129,8   9));
    //resize(ransacPlano,ransacPlano,Size(),patchResize,patchResize,INTER_LINEAR);
    //blur(ransacPlano, ransacPlano, Size(3, 3), Point(-1,-1));
	...
	//cout << ransacPlano.rows << " " << ransacPlano.cols << endl;
	Mat ransacQuery = imread(filename2,IMREAD_GRAYSCALE);
    copyMakeBorder(ransacQuery,ransacQuery,50,50,50,50,BORDER_CONSTANT,255);
    //resize(ransacQuery,ransacQuery,Size(),patchResize,patchResize,INTER_LINEAR);
    //blur(ransacQuery, ransacQuery, Size(3, 3), Point(-1,-1));
    //resize(ransacQuery,ransacQuery,Size(105,57));
    //blur( ransacQuery, ransacQuery, Size(3,3), Point(-1,-1) );
...
float ransacCompare(cv:: Mat img_object, cv:: Mat img_scene){
    ...
    imwrite("img_object_points.jpg",img_object);
    ...
    imwrite("img_scene_points.jpg",img_scene);
    ...
    const float descriptorDistanceRatio = 0.9;
    
    for (int i = 0; i < matches.size(); ++i){
        if (matches[i][0].distance < descriptorDistanceRatio * matches[i][1].distance){
        //cout<<"the i and j are "<<i<<" "<<matches[i].size()<<"\n";
    ...
    imwrite("ans.jpg",warped);
    perspectiveTransform(obj,proj,H);


    std::vector<DMatch> very_good_matches;
    double tolerance =  max(1,max(img_scene.rows,img_scene.cols)/5);
    double tolerance_2 = max(1,max(img_object.rows,img_object.cols)/5);
    ...
    string write_patch = "correspondences.jpg";
